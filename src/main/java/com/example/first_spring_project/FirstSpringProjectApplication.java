...
import com.example.first_spring_project.repositories.PatientRepository;
...
public class FirstSpringProjectApplication implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            patientRepository.save(new Patient(null, "omar", new Date(), Math.random() > 0.5 ? true : false, (int)(Math.random()*100)));
        }
        ...
        List<Patient> patientList = patientRepository.chercherPatients("%om%", 40);
